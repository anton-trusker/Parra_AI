...
export default function ScanProgressDialog({ open, onComplete, simulatedWine }: ScanProgressDialogProps) {
  ...
  useEffect(() => {
    ...
    const t1 = setTimeout(() => { setStage('searching'); setProgress(20); }, 600);
    const t2 = setTimeout(() => setProgress(50), 1000);
    const t3 = setTimeout(() => setProgress(80), 1500);
    const t4 = setTimeout(() => {
      if (simulatedWine) { setStage('found'); setProgress(100); }
      else { setStage('not-found'); }
    }, 2200);
    const t5 = setTimeout(() => {
      if (simulatedWine) { onComplete(simulatedWine, 92 + Math.random() * 6); }
      else { onComplete(null, 0); }
    }, 3000);
      ...
      <div className="relative w-[280px] bg-card border border-border rounded-2xl p-6 text-center shadow-2xl">
        ...
        {stage === 'found' && (
          <>
            ...
            {simulatedWine && <p className="text-sm text-muted-foreground mt-1 truncate">{simulatedWine.name}</p>}
          </>
        )}
        {stage === 'not-found' && (
          <>
