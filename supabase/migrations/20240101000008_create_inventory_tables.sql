...
CREATE TABLE IF NOT EXISTS inventory_items (
  ...
  product_id      UUID NOT NULL REFERENCES wines(id) ON DELETE RESTRICT, -- Changed products(id) to wines(id) because products table name is wines in phase-01
  location_id     UUID REFERENCES locations(id),
...
CREATE TABLE IF NOT EXISTS inventory_movements (
  id              UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  product_id      UUID NOT NULL REFERENCES wines(id) ON DELETE RESTRICT, -- Changed products(id) to wines(id)
...
CREATE TABLE IF NOT EXISTS current_stock (
  id              UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  product_id      UUID NOT NULL REFERENCES wines(id) ON DELETE CASCADE UNIQUE, -- Changed products(id) to wines(id)
  location_id     UUID REFERENCES locations(id),
  quantity        NUMERIC NOT NULL DEFAULT 0,
...
-- Note: 'roles' table referenced here might be 'app_roles_config' or 'user_roles' from phase-01
-- Phase-01 uses 'user_roles' table with 'role' enum column.
-- The query below uses `JOIN roles r` which doesn't exist in phase-01.
-- Phase-01 has `user_roles` with `role` column (enum).
-- So I need to adapt the policy.
...
-- Adapted Policy for Phase-01 Schema:
CREATE POLICY "Admins can view all sessions" ON inventory_sessions
  FOR SELECT USING (
    EXISTS (
